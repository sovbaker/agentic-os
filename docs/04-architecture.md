...
| Хостинг 🔁 | РФ-регион при рынке РФ | 152-ФЗ, локализация персональных данных |
...
/apps
  ...
  /web               — публичный рендер UISpec (шаринг)
/services
  /api               — BFF: REST + SSE
...
/packages
  /contracts         — UISpec, Job, Entity/Fact, Tool manifest (zod, общие для клиента и сервера)
...
Ключевое: `packages/contracts` — единственный источник правды для схем, используется и клиентом, и сервером, и генерацией. Расхождение контрактов между клиентом и сервером — самая частая причина боли в SDUI-системах.
