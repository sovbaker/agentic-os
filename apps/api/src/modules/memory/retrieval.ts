...
/**
 * Персональный контекст для модели одним текстовым блоком.
 *
 * Два свойства делают его именно блоком, а не набором полей в промпте:
 * он **одинаков для всех ролей** и **редко меняется между ходами** — то
 * есть это единственная часть запроса, которую есть смысл кэшировать.
 * Системная инструкция для кэша слишком коротка, а текст пользователя
 * уникален каждый раз.
 *
 * Значения фактов сюда попадают как есть. Это безопасно только потому, что
 * в графе не бывает сырого недоверенного текста: карантин пропускает наружу
 * структуру, а не содержимое письма или страницы.
 */
export async function renderContext(userId: string, intent = ''): Promise<string | null> {
  const facts = await retrieve({ userId, intent, limit: 40, minConfidence: 0.4 });
  if (facts.length === 0) return null;

  const lines = facts.map((f) => {
    const value = typeof f.value === 'string' && f.value ? ` → ${f.value}` : '';
    // Источник и уверенность идут рядом со фактом: модель должна видеть,
    // что предположение архетипа — не то же самое, что сказанное вслух.
    return `- ${f.label} · ${f.predicate}${value} [${f.source}, ${f.confidence.toFixed(2)}]`;
  });

  return `Что известно о пользователе (граф, ${facts.length} фактов):\n${lines.join('\n')}`;
}
