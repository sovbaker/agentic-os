...
export const ComponentType = z.enum([
  ...
  /*
   * Агентность — то, ради чего продукт существует.
   *
   * Реестр из 49 компонентов не умел сказать ни «я это сделал», ни «жду
   * ответа от третьей стороны», ни «вот что я собираюсь изменить». Пока их
   * нет, единственное отличие продукта от списка дел невыразимо, и любой
   * сгенерированный экран описывает задачу, а не работу над ней.
   *
   * Ось у всех четырёх одна и та же — ЧЕЙ СЕЙЧАС ХОД: агента, мира или
   * человека. Это не метафора, а состояние задачи на сервере.
   */
...
/**
 * Иконки — закрытое перечисление, как и компоненты.
 *
 * Раньше `icon` принимал `glyph: string`, и сервер присылал эмодзи. Эмодзи
 * как иконочная система означает чужой рисунок, чужую метрику и цвет,
 * которым нельзя управлять. Здесь имя разрешается в нарисованную форму на
 * клиенте, поэтому подсунуть произвольный символ физически нельзя.
 *
 * Набор ограничен тем, что честно рисуется геометрией View без нативных
 * зависимостей: это цена отказа от эмодзи сегодня, а не навсегда.
 */
